...
virt_addr_t vmm_kmap(phys_addr_t phys_addr, const size_t size, virt_addr_t virt_addr,
  ...
  uintptr_t start_addr = vaddr;

  while (found_free_pages < pages_required && vaddr < KSPACE_END) {
    if (is_virt_addr_used((virt_addr_t)vaddr)) {
      // reset
      vaddr += PAGE_SIZE;
      found_free_pages = 0;
      start_addr = vaddr;
  ...
  for (size_t i = 0; i < pages_required; ++i) {
    ...
    if (!create_page_table_entry((virt_addr_t)addr, entry)) {
      kpanic("vmm_kmap: failed to create page table entry");
    }
    invlpg((virt_addr_t)page_addr);
  }

  return (virt_addr_t)(start_addr + phys_offset);
}
...
void vmm_kunmap(virt_addr_t virt_addr, const size_t size) {
  ...
  // should
  const uintptr_t end_addr = round_up_addr((uintptr_t)vaddr + size);
  vaddr = round_down_addr(vaddr);
  ...
  for (; vaddr < end_addr; vaddr += PAGE_SIZE) {
    const struct ptindices is = virt_addr_to_ptindices((virt_addr_t)vaddr);
    get_PT(is.pml4t_i, is.pdpt_i, is.pd_i)[is.pt_i].present = false;
#ifdef VERBOSE_VMM
    printf("Unmapped %#18zx\n", virt_addr);
#endif
    invlpg((virt_addr_t)vaddr);
  }
}
